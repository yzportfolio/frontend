...
Related.prototype.popularInTagOverride = function() {
    ...
    var whitelistedTags = [
        ...
        popularInTags = config.page.isPaidContent
            ? pageTags
...
Related.prototype.renderRelatedComponent = function() {
    var relatedUrl,
        ...
        container,
        fetchRelated =
            config.switches.relatedContent && config.page.showRelatedContent;
    if (config.page && config.page.hasStoryPackage) {
        ...
        if (container) {
            ...
            relatedUrl =
                popularInTag || '/related/' + config.page.pageId + '.json';

            if (opts.excludeTags && opts.excludeTags.length) {
                relatedUrl +=
                    '?' +
                    map(opts.excludeTags, function(tag) {
            ...
            lazyload(relatedUrl, {
                container: container,
                    ...
                    var relatedContainer = container.querySelector(
                        '.related-content'
                    );

...
function init() {
        ...
        if (match.pageType === 'stats') {
            ...
            renderNav(match, function(resp, $nav, endpoint) {
                ...
                if (resp.hasStarted && $nav) {
                    var statsUrl = $('.tab--stats a', $nav)
                        .attr('href')
                        .replace(/^.*\/\/[^\/]+/, '');

                    $.create(
                        '<div class="match-stats__container"></div>'
                    ).each(function(container) {
                        football
                            .statsFor(statsUrl)
                            .fetch(container)
    ...
    page.isLiveClockwatch(function() {
        var ml = new MatchListLive(
            'match-day',
            page.isCompetition() || 'premierleague',
            config.dateFromSlug()
        ),
            $img = $('.media-primary'),
            $matchListContainer = $.create(
                '<div class="football-matches__container" data-link-name="football-matches-clockwatch"></div>'
            ).css({
    ...
    // Binding
    bean.on(document.body, 'click', '.js-show-more-football-matches', function(
        e
    ) {
        e.preventDefault();
            ...
            .then(function(resp) {
                $.create(resp.html).each(function(html) {
                    var htmlContainer = document.querySelector(
                        '[data-show-more-contains="' +
                            el.getAttribute('data-puts-more-into') +
                            '"]'
                    );
